...
| Dívida: validação inline → serializers (ADR-0009) + auditoria de stats (eixos de gate_coordinates com precedência, `n`/`rcv` por canal, `FileStatsView` reusando `calculate_cytometry_metrics`) | `*/serializers.py`, `*/views.py`, `*/tests.py`, `utils/density.py`, `analytics/tasks.py` | `refactor/inline-validation-serializers` | devin | 2026-09 |
